...
STATE_DIR = Path(os.environ.get(
    "GENORA_YOUTUBE_STATE_DIR",
    str(Path.home() / ".cache" / "genora-youtube-agent"),
)).expanduser()
TMP_DIR = STATE_DIR / "cache"
TOKEN_PATH = Path(os.environ.get(
    "GENORA_YOUTUBE_OAUTH_TOKEN", str(STATE_DIR / "youtube_oauth_token.json")
)).expanduser()
CLIENT_SECRETS_PATH = Path(os.environ.get(
    "GENORA_YOUTUBE_CLIENT_SECRETS", str(STATE_DIR / "youtube_client_secrets.json")
)).expanduser()
...
def get_data_api_service(api_key=None):
    ...
    if not api_key:
        return None, {
            ...
            "fix": (
                "Set YOUTUBE_API_KEY in the private runtime environment. "
...
def check_auth(auth_type):
    ...
    if auth_type == "api_key":
        ...
        return {"status": "missing", "type": "api_key",
                "fix": "Set YOUTUBE_API_KEY in the private Genora runtime environment"}
