...
export async function GET(request:Request) {
  try {
    ...
    if(type&&!["chat","image","video","song"].includes(type)) return jsonError("Некорректный тип");
    if(!Number.isFinite(page)||page<1||page>10_000) return jsonError("Некорректная страница");
    return Response.json(await getAdminDashboardData(from,to,userId,model,provider,page,type));
  } catch(error) {
    ...
    return jsonError("Не удалось загрузить админку",500);
  }
}
