...
import { executeVideoJob, insertVideoJob, publicVideoJob } from "@/lib/server/video-jobs";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (style !== "auto" && !videoStyleById(locale, style)) return jsonError(appCopy.imageParamsRequired);
    ...
    if (!(catalogModel.resolutions ?? []).includes(resolution)) return jsonError(appCopy.imageParamsRequired);
    if (!(catalogModel.aspect_ratios ?? []).includes(aspectRatio)) return jsonError(appCopy.imageParamsRequired);
    ...
    if (mode === "motion-control") {
      ...
      if (billed == null) return jsonError(appCopy.imageParamsRequired);
      duration = billed;
    }
    ...
    const setup = await withTransaction(async (client) => {
      ...
      let conversation: { id: string; title: string; updated_at: Date } | undefined;
      if (requestedConversationId) {
        const owned = await client.query<{ id: string; title: string; updated_at: Date }>("SELECT id,title,updated_at FROM image_conversations WHERE id=$1 AND user_id=$2 AND deleted_at IS NULL FOR UPDATE", [requestedConversationId, user.id]);
        conversation = owned.rows[0];
        if (!conversation) throw new Error("IMAGE_CONVERSATION_NOT_FOUND");
      } else {
        const created = await client.query<{ id: string; title: string; updated_at: Date }>(`INSERT INTO image_conversations(user_id,title)
          VALUES($1,$2) RETURNING id,title,updated_at`, [user.id, titleFromPrompt(prompt, videoAgent?.name ?? copy.newImageConversation)]);
