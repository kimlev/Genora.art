...
function Connection({settings,setSettings,requirements,busy,onSave,onConnect,onDisconnect,onNext}:{settings:Settings;setSettings:(value:Settings)=>void;requirements:Workspace["requirements"];busy:boolean;onSave:()=>void;onConnect:()=>void;onDisconnect:()=>void;onNext:()=>void}){
  ...
  return <div className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
    ...
    <Card title="Skills и автоматизация" icon={<Bot className="size-4"/>}>
      ...
      <label className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-100 p-3 text-xs text-slate-900 dark:border-slate-800 dark:bg-slate-950/60 dark:text-slate-100"><input type="checkbox" checked={settings.automationEnabled} onChange={(event)=>set("automationEnabled",event.target.checked)}/><span><b className="block text-slate-900 dark:text-slate-200">Фоновая автоматизация</b><span className="text-slate-600 dark:text-slate-500">Research, план и обновление аналитики по расписанию</span></span></label>
...
function Card({title,icon,children}:{title:string;icon?:ReactNode;children:ReactNode}){return <section className="space-y-4 rounded-2xl border border-slate-200 bg-slate-100 p-5 text-slate-900 dark:border-slate-800 dark:bg-slate-900/40 dark:text-slate-100"><header className="flex items-center gap-2"><span className="text-orange-500 dark:text-orange-300">{icon}</span><h2 className="font-semibold">{title}</h2></header>{children}</section>}
...
function Select({label,value,onChange,options}:{label:string;value:string;onChange:(value:string)=>void;options:string[][]}){return <AdminSelect label={label} value={value} options={options.map(([id,name])=>({value:id,label:name}))} emptyLabel={options[0]?.[1]??"Выберите значение"} onChange={onChange} clearable={false} className="w-full"/>}
function Status({ok,label}:{ok:boolean;label:string}){return <span className={cn("inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-semibold",ok?"bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300":"bg-slate-200 text-slate-700 dark:bg-slate-700 dark:text-slate-300")}><span className={cn("size-1.5 rounded-full",ok?"bg-emerald-500 dark:bg-emerald-400":"bg-slate-500")}/>{label}</span>}
function Requirement({label,ok}:{label:string;ok:boolean}){return <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-900 dark:border-slate-800 dark:bg-slate-950/50 dark:text-slate-100"><span>{label}</span><Status ok={ok} label={ok?"готово":"нужно настроить"}/></div>}
function CopyValue({value}:{value:string}){return <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 font-mono text-[11px] text-slate-900 break-all dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300">{value}</div>}
function Setup({lines}:{lines:string[]}){return <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-slate-900 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-slate-100"><p className="mb-2 text-xs font-semibold text-amber-900 dark:text-amber-100">Переменные сервера</p><div className="space-y-1 font-mono text-[11px] text-amber-800 dark:text-amber-200">{lines.map((line)=><p key={line}>{line}</p>)}</div></div>}
