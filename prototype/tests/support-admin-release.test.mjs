...
const devWorkflow = await readFile(new URL("../../.github/workflows/deploy-dev.yml", import.meta.url), "utf8");
...
test("dev deploy recreates the support worker with the current application image", () => {
  assert.match(devWorkflow, /docker compose up -d --build app support-worker/);
  assert.match(devWorkflow, /docker compose ps app support-worker/);
});
