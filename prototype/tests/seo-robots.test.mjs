...
test("dev keeps public crawlers blocked but exposes its sitemap to Blogoro", () => {
  const robots = readFileSync(join(root, "src/app/robots.ts"), "utf8");
  const sitemap = readFileSync(join(root, "src/app/sitemap.ts"), "utf8");
  assert.match(robots, /userAgent: "ai-blog-analyzer"/);
  assert.match(robots, /userAgent: "\*", disallow: "\/", allow: "\/sitemap\.xml"/);
