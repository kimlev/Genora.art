...
3.6. Dev-база копируется ежедневно. Архив проверяется на читаемость, шифруется AES-256-GCM, отправляется в отдельный bucket HOSTKEY S3 в Нидерландах и проверяется обратным чтением и контрольной суммой. Локальный файл на VPS удаляется после подтверждения копии. Плановый срок хранения — 30 дней: ежедневная ротация начинается с 29-го дня, правило S3 предусматривает удаление после 30 дней. Полное тестовое восстановление базы не заменяется проверкой архива. Для будущей рабочей среды подготовлен отдельный путь, но её копирование будет запущено отдельно.
