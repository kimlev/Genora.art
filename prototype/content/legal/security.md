...
3.6. Копия dev-базы создаётся ежедневно и проверяется на читаемость каталога архива. Копии хранятся на том же VPS в Хельсинки с доступом только для администратора сервера и автоматически удаляются через 30 дней. Полное тестовое восстановление не входит в ежедневную проверку; хранение на том же сервере не защищает от утраты VPS. Для будущей рабочей среды режим копирования будет проверен и опубликован отдельно.
