...
**Непрерывность:** dev-база копируется ежедневно; архив проверяется на читаемость, хранится на том же VPS HOSTKEY в Хельсинки с доступом только для администратора и удаляется через 30 дней. Это не заменяет внешнюю копию или полное тестовое восстановление. Режим будущей рабочей среды подтверждается отдельно.
